...
class MCTformerV2(VisionTransformer):
    ...
    def forward(self, x, return_att=False, n_layers=12, attention_type='fused', forward_feat=False, use_fine=False):
        w, h = x.shape[2:]
        ...
        if attention_type == 'fused':  # patchcam + mct attention 出圖
            cams = mtatt * feature_map  # B * C * 14 * 14
        elif attention_type == 'patchcam':    # only patchcam
            cams = feature_map 
        else:      # only mct attention
            cams = mtatt

        patch_attn = attn_weights[:, :, self.num_classes:, self.num_classes:] 
        patch_attn_sum = torch.sum(patch_attn, dim=0) # B * 196 * 196

        fine_cam = torch.matmul(patch_attn_sum.unsqueeze(1), cams.view(cams.shape[0], cams.shape[1], -1, 1)). \
        ...
        elif forward_feat and not use_fine:
            return x_cls_logits, x_patch_logits, feat, coarse_cam
        elif forward_feat and use_fine:
            return x_cls_logits, x_patch_logits, feat, fine_cam
        else:
